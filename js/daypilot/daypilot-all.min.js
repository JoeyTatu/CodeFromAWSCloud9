...
(function() {
    ...
    DayPilot.Http.ajax = function($F) {
        ...
        var $G = typeof $F.data === "object";
        var $H = $F.data;
        var $I = $F.method || "GET";
        var $J = $F.success || function() {};
        var $K = $F.error || function() {};
        var $z = $F.url;
        var $L = $F.contentType || ($G ? "application/json" : "text/plain");
        $D.open($I, $z, true);
        $D.setRequestHeader('Content-type', $L);
        $D.onreadystatechange = function() {
            if ($D.readyState !== 4) {
                return;
            };
            ...
            var $M = {};
            $M.request = $D;
            $M.data = JSON.parse($D.responseText);
            $J($M);
        };
        if ($D.readyState === 4) {
            return;
    ...
    DayPilot.Util = {};
    DayPilot.Util.addClass = function($r, client_name) {
        if (!$r) {
            return;
        };
        if (!$r.className) {
            $r.className = client_name;
            return;
        };
        var $N = new RegExp("(^|\\s)" + client_name + "($|\\s)");
        if (!$N.test($r.className)) {
            $r.className = $r.className + ' ' + client_name;
        }
    };
    DayPilot.Util.removeClass = function($r, client_name) {
        if (!$r) {
            return;
        };
        var $N = new RegExp("(^|\\s)" + client_name + "($|\\s)");
        $r.className = $r.className.replace($N, ' ').replace(/^\s\s*/, '').replace(/\s\s*$/, '');
    };
    DayPilot.Util.isNullOrUndefined = function($O) {
        return $O === null || typeof $O === "undefined";
    };
    DayPilot.Exception = function($P) {
        return new Error($P);
    };
    DayPilot.Locale = function(id, $Q) {
        this.id = id;
        ...
        this.weekStarts = 0;
        if ($Q) {
            for (var client_name in $Q) {
                this[client_name] = $Q[client_name];
            }
        }
    };
    DayPilot.Locale.all = {};
    DayPilot.Locale.find = function(id) {
        ...
        var $R = id.toLowerCase();
        if ($R.length > 2) {
            $R[2] = '-';
        };
        return DayPilot.Locale.all[$R];
    };
    DayPilot.Locale.register = function($S) {
        DayPilot.Locale.all[$S.id] = $S;
    };
    ...
    DayPilot.Date = function($T, $U) {
        if ($T instanceof DayPilot.Date) {
            return $T;
        };
        var $V;
        if (DayPilot.Util.isNullOrUndefined($T)) {
            $V = DayPilot.DateUtil.fromLocal().getTime();
            $T = $V;
        };
        var $W = DayPilot.Date.Cache.Ctor;
        if ($W[$T]) {
        ...
        var $X = false;
        if (typeof $T === "string") {
            $V = DayPilot.DateUtil.fromStringSortable($T, $U).getTime();
            $X = true;
            ...
            if ($U) {
                $V = DayPilot.DateUtil.fromLocal($T).getTime();
            } else {
                $V = $T.getTime();
            }
        } else {
        ...
        var $Y = ticksToSortable($V);
        if ($W[$Y]) {
            return $W[$Y];
        };
        $W[$Y] = this;
        $W[$V] = this;
        if ($X && $Y !== $T && DayPilot.DateUtil.hasTzSpec($T)) {
            $W[$T] = this;
        };
    ...
    DayPilot.Date.prototype.addDays = function($Z) {
        return new DayPilot.Date(this.ticks + $Z * 24 * 60 * 60 * 1000);
    };
    DayPilot.Date.prototype.addHours = function($00) {
        return this.addTime($00 * 60 * 60 * 1000);
    };
    DayPilot.Date.prototype.addMilliseconds = function($01) {
        return this.addTime($01);
    };
    DayPilot.Date.prototype.addMinutes = function($02) {
        return this.addTime($02 * 60 * 1000);
    };
    DayPilot.Date.prototype.addMonths = function($03) {
        ...
        d.setUTCFullYear(y);
        d.setUTCMonth(m - 1);
        var $04 = new DayPilot.Date(d).daysInMonth();
        d.setUTCDate(Math.min($04, $T.getUTCDate()));
        return new DayPilot.Date(d);
    };
    DayPilot.Date.prototype.addSeconds = function($05) {
        return this.addTime($05 * 1000);
    };
    DayPilot.Date.prototype.addTime = function($V) {
        if ($V instanceof DayPilot.Duration) {
            $V = $V.ticks;
        };
        return new DayPilot.Date(this.ticks + $V);
    };
    DayPilot.Date.prototype.addYears = function($06) {
        var $07 = new Date(this.ticks);
        var d = new Date(this.ticks);
        var y = this.getYear() + $06;
        var m = this.getMonth();
        d.setUTCDate(1);
        d.setUTCFullYear(y);
        d.setUTCMonth(m);
        var $04 = new DayPilot.Date(d).daysInMonth();
        d.setUTCDate(Math.min($04, $07.getUTCDate()));
        return new DayPilot.Date(d);
    };
    ...
    DayPilot.Date.prototype.daysInMonth = function() {
        var $T = new Date(this.ticks);
        var $08 = $T.getUTCMonth() + 1;
        var $09 = $T.getUTCFullYear();
        var m = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
        if ($08 !== 2) return m[$08 - 1];
        if ($09 % 4 !== 0) return m[1];
        if ($09 % 100 === 0 && $09 % 400 !== 0) return m[1];
        return m[1] + 1;
    };
    ...
    DayPilot.Date.prototype.equals = function($0a) {
        if ($0a === null) {
            return false;
        };
        if ($0a instanceof DayPilot.Date) {
            return this === $0a;
        } else {
    ...
    DayPilot.Date.prototype.firstDayOfWeek = function($0b) {
        var d = this;
        if ($0b instanceof DayPilot.Locale) {
            $0b = $0b.weekStarts;
        } else if (typeof $0b === "string" && DayPilot.Locale.find($0b)) {
            var $S = DayPilot.Locale.find($0b);
            $0b = $S.weekStarts;
        } else {
            $0b = $0b || 0;
        };
    ...
    DayPilot.Date.prototype.getTimePart = function() {
        var $0d = this.getDatePart();
        return DayPilot.DateUtil.diff(this, $0d);
    };
    ...
    DayPilot.Date.prototype.weekNumber = function() {
        var $0e = this.firstDayOfYear();
        var $Z = (this.getTime() - $0e.getTime()) / 86400000;
        return Math.ceil(($Z + $0e.dayOfWeek() + 1) / 7);
    };
    DayPilot.Date.prototype.weekNumberISO = function() {
        var $0f = false;
        var $0g = this.dayOfYear();
        var $0h = this.firstDayOfYear().dayOfWeek();
        var $0i = this.firstDayOfYear().addYears(1).addDays(-1).dayOfWeek();
        if ($0h === 0) {
            $0h = 7;
        };
        if ($0i === 0) {
        ...
        if ($0l === 0) {
            $0l = this.firstDayOfYear().addDays(-1).weekNumberISO();
        };
        return $0l;
    };
    DayPilot.Date.prototype.toDateLocal = function() {
        ...
        d.setMinutes($T.getUTCMinutes());
        d.setSeconds($T.getUTCSeconds());
        d.setMilliseconds($T.getUTCMilliseconds());
        return d;
    };
    ...
    DayPilot.Date.prototype.toString = function($0m, $S) {
        if (!$0m) {
            return this.toStringSortable();
        };
        return new $0n($0m, $S).print(this);
    };
    DayPilot.Date.prototype.toStringSortable = function() {
        return ticksToSortable(this.ticks);
    };

    function ticksToSortable($V) {
        var $W = DayPilot.Date.Cache.Ticks;
        if ($W[$V]) {
        ...
        var $0p = d.getUTCSeconds();
        if ($0p < 10) $0p = "0" + $0p;
        var $0q = d.getUTCMinutes();
        if ($0q < 10) $0q = "0" + $0q;
        var $0r = d.getUTCHours();
        if ($0r < 10) $0r = "0" + $0r;
        var $0c = d.getUTCDate();
        if ($0c < 10) $0c = "0" + $0c;
        var $08 = d.getUTCMonth() + 1;
        if ($08 < 10) $08 = "0" + $08;
        var $09 = d.getUTCFullYear();
        if ($09 <= 0) {
        ...
        var $h = $09 + "-" + $08 + "-" + $0c + 'T' + $0r + ":" + $0q + ":" + $0p + $0o;
        $W[$V] = $h;
        return $h;
    };
    DayPilot.Date.parse = function(str, $0m, $S) {
        var p = new $0n($0m, $S);
        return p.parse(str);
    };
    var $0s = 0;
    ...
    DayPilot.Date.fromYearMonthDay = function($09, $08, $0c) {
        $08 = $08 || 1;
        $0c = $0c || 1;
        var d = new Date(0);
        d.setUTCFullYear($09);
    ...
    DayPilot.Duration = function($V) {
        var d = this;
        var $0c = 1000 * 60 * 60 * 24;
        var $0r = 1000 * 60 * 60;
        var $0q = 1000 * 60;
        var $0p = 1000;
        if (arguments.length === 2) {
            ...
            $V = end.getTime() - $u.getTime();
        };
        this.ticks = $V;
        if (DayPilot.Date.Cache.DurationCtor["" + $V]) {
            return DayPilot.Date.Cache.DurationCtor["" + $V];
        };
        DayPilot.Date.Cache.DurationCtor["" + $V] = this;
        this.toString = function($0m) {
            if (!$0m) {
        ...
        this.hours = function() {
            var $0t = d.ticks - d.days() * $0c;
            return Math.floor($0t / $0r);
        };
        this.minutes = function() {
            var $0u = d.ticks - Math.floor(d.totalHours()) * $0r;
            return Math.floor($0u / $0q);
        };
        this.seconds = function() {
            var $0v = d.ticks - Math.floor(d.totalMinutes()) * $0q;
            return Math.floor($0v / $0p);
        };
    ...
    DayPilot.DateUtil = {};
    DayPilot.DateUtil.fromStringSortable = function($0w, $U) {
        if (!$0w) {
        ...
        var long = $0x > 19;
        if (!$T && !$0y && !long) {
            throw "Invalid string format (use '2010-01-01' or '2010-01-01T00:00:00'): " + $0w;
        };
        if (DayPilot.Date.Cache.Parsing[$0w] && !$U) {
            DayPilot.Stats.cacheHitsParsing += 1;
            return DayPilot.Date.Cache.Parsing[$0w];
        };
        var $09 = $0w.substring(0, 4);
        var $08 = $0w.substring(5, 7);
        var $0c = $0w.substring(8, 10);
        var d = new Date(0);
        d.setUTCFullYear($09, $08 - 1, $0c);
        if ($T) {
            DayPilot.Date.Cache.Parsing[$0w] = d;
            return d;
        };
        var $00 = $0w.substring(11, 13);
        var $02 = $0w.substring(14, 16);
        var $05 = $0w.substring(17, 19);
        d.setUTCHours($00);
        d.setUTCMinutes($02);
        d.setUTCSeconds($05);
        if ($0y) {
        ...
        var $0A = 0;
        if ($0z === ".") {
            var ms = parseInt($0w.substring(20, 23));
            d.setUTCMilliseconds(ms);
            $0A = DayPilot.DateUtil.getTzOffsetMinutes($0w.substring(23));
        } else {
            $0A = DayPilot.DateUtil.getTzOffsetMinutes($0w.substring(19));
        };
    ...
    DayPilot.DateUtil.getTzOffsetMinutes = function($0w) {
        if (DayPilot.Util.isNullOrUndefined($0w) || $0w === "") {
            return 0;
        };
        if ($0w === "Z") {
            return 0;
        ...
        var $0z = $0w[0];
        var $0B = parseInt($0w.substring(1, 3));
        var $0C = parseInt($0w.substring(4));
        var $0A = $0B * 60 + $0C;
        if ($0z === "-") {
            return -$0A;
        } else if ($0z === "+") {
            ...
            throw "Invalid timezone spec: " + $0w;
        }
    };
    DayPilot.DateUtil.hasTzSpec = function($0w) {
        if ($0w.indexOf("+")) {
            return true;
        };
        if ($0w.indexOf("-")) {
    ...
    DayPilot.DateUtil.daysDiff = function($0e, $0p) {
        ($0e && $0p) || (function() {
            throw "two parameters required";
        })();
        $0e = new DayPilot.Date($0e);
        $0p = new DayPilot.Date($0p);
        if ($0e.getTime() > $0p.getTime()) {
            return null;
        };
        var i = 0;
        var $0D = $0e.getDatePart();
        var $0E = $0p.getDatePart();
        while ($0D < $0E) {
            $0D = $0D.addDays(1);
            i++;
        };
    ...
    DayPilot.DateUtil.daysSpan = function($0e, $0p) {
        ($0e && $0p) || (function() {
            throw "two parameters required";
        })();
        $0e = new DayPilot.Date($0e);
        ...
        var $0F = DayPilot.DateUtil.daysDiff($0e, $0p);
        if ($0p == $0p.getDatePart()) {
            $0F--;
        };
        return $0F;
    };
    DayPilot.DateUtil.diff = function($0e, $0p) {
        if (!($0e && $0p && $0e.getTime && $0p.getTime)) {
            throw "Both compared objects must be Date objects (DayPilot.Date.diff).";
        };
        return $0e.getTime() - $0p.getTime();
    };
    DayPilot.DateUtil.fromLocal = function($0G) {
        if (!$0G) {
            $0G = new Date();
        };
        var d = new Date();
    ...
    DayPilot.DateUtil.hours = function($T, $0H) {
        var $0q = $T.getUTCMinutes();
        if ($0q < 10) $0q = "0" + $0q;
        var $0r = $T.getUTCHours();
        if ($0H) {
            var am = $0r < 12;
            var $0r = $0r % 12;
            if ($0r === 0) {
    ...
    DayPilot.DateUtil.max = function($0e, $0p) {
        if ($0e.getTime() > $0p.getTime()) {
            return $0e;
        } else {
            return $0p;
    ...
    DayPilot.DateUtil.min = function($0e, $0p) {
        if ($0e.getTime() < $0p.getTime()) {
            return $0e;
        } else {
            return $0p;
    ...
    var $0n = function($0m, $S) {
        if (typeof $S === "string") {
            $S = DayPilot.Locale.find($S);
        };
        var $S = $S || DayPilot.Locale.US;
        var $0J = [{
            "seq": "yyyy",
            "expr": "[0-9]{4,4\u007d",
            "str": function(d) {
            ...
            "expr": "[^\\s0-9]*",
            "str": function(d) {
                var r = $S.monthNames[d.getMonth()];
                return r;
            },
            "transform": function($0K) {
                var $p = DayPilot.indexOf($S.monthNames, $0K, $0L);
                if ($p < 0) {
                    return null;
                };
                return $p + 1;
            ...
            "expr": "[^\\s0-9]*",
            "str": function(d) {
                var r = $S.monthNamesShort[d.getMonth()];
                return r;
            },
            "transform": function($0K) {
                var $p = DayPilot.indexOf($S.monthNamesShort, $0K, $0L);
                if ($p < 0) {
                    return null;
                };
                return $p + 1;
            ...
            "expr": "[^\\s0-9]*",
            "str": function(d) {
                var r = $S.dayNames[d.getDayOfWeek()];
                return r;
            }
        }, {
            "seq": "ddd",
            "expr": "[^\\s0-9]*",
            "str": function(d) {
                var r = $S.dayNamesShort[d.getDayOfWeek()];
                return r;
            }
        }, {
            "seq": "dd",
        ...
        this.findSequence = function($0O) {
            function defaultTransform($Y) {
                return parseInt($Y);
            };
            var $p = $0m.indexOf($0O);
            if ($p === -1) {
            ...
            return {
                "findValue": function($0K) {
                    ...
                    try {
                        ...
                        $0Q = $0Q || defaultTransform;
                        return $0Q($q[1]);
                    } catch (e) {
                        throw "unable to create regex from: " + $0P;
                    }
                }
            };
        };
        ...
        this.parse = function($0K) {
            var $09 = this.year.findValue($0K);
            if (!$09) {
                return null;
            };
            var $08 = this.month.findValue($0K);
            if (DayPilot.Util.isNullOrUndefined($08)) {
                return null;
            };
            if ($08 > 12 || $08 < 1) {
                return null;
            };
            var $0c = this.day.findValue($0K);
            var $0Z = DayPilot.Date.fromYearMonthDay($09, $08).daysInMonth();
            if ($0c < 1 || $0c > $0Z) {
                return null;
            };
            var $00 = this.hours ? this.hours.findValue($0K) : 0;
            var $02 = this.minutes ? this.minutes.findValue($0K) : 0;
            var $05 = this.seconds ? this.seconds.findValue($0K) : 0;
            var d = new Date();
            d.setUTCFullYear($09, $08 - 1, $0c);
            d.setUTCHours($00);
            d.setUTCMinutes($02);
    ...
    DayPilot.Event = function($H, $10, $11) {
        var e = this;
        this.calendar = $10;
        this.data = $H ? $H : {};
        this.part = $11 ? $11 : {};
        ...
        this.start = function($O) {
            if (typeof $O === 'undefined') {
                ...
                this.temp().start = new DayPilot.Date($O);
            }
        };
        this.end = function($O) {
            if (typeof $O === 'undefined') {
                ...
                this.temp().end = new DayPilot.Date($O);
            }
        };
        ...
        this.tag = function($14) {
            ...
            var $16 = e.calendar.tagFields;
            var $p = -1;
            for (var i = 0; i < $16.length; i++) {
                if ($14 === $16[i]) $p = i;
            };
            if ($p === -1) {
                throw "Field client_name not found.";
            };
            return $15[$p];
        };
        this.client = {};
        this.client.innerHTML = function($O) {
            if (typeof $O === 'undefined') {
        ...
        this.client.cssClass = function($O) {
            if (typeof $O === 'undefined') {
        ...
        this.client.toolTip = function($O) {
            if (typeof $O === 'undefined') {
        ...
        this.client.barVisible = function($O) {
            if (typeof $O === 'undefined') {
        ...
        this.client.backColor = function($O) {
            if (typeof $O === 'undefined') {
        ...
        this.client.borderColor = function($O) {
            if (typeof $O === 'undefined') {
...
(function() {
    ...
    function quote($0w) {
        $17.lastIndex = 0;
        return $17.test($0w) ? '"' + $0w.replace($17, function(a) {
            var c = $1a[a];
            if (typeof c === 'string') {
                return c;
            };
    ...
    function str($x, $1c) {
        var i, k, v, length, $1d = $18,
            $1e, $Y = $1c[$x];
        if ($Y && typeof $Y === 'object' && typeof $Y.toJSON2 === 'function') {
            $Y = $Y.toJSON2($x);
        } else if ($Y && typeof $Y === 'object' && typeof $Y.toJSON === 'function' && !$Y.ignoreToJSON) {
            $Y = $Y.toJSON($x);
        };
        if (typeof $1b === 'function') {
            $Y = $1b.call($1c, $x, $Y);
        };
        switch (typeof $Y) {
            case 'string':
    ...
    if (typeof DayPilot.JSON.stringify !== 'function') {
        DayPilot.JSON.stringify = function($Y, $1f, $1g) {
            var i;
            $18 = '';
            $19 = '';
            if (typeof $1g === 'number') {
            ...
            } else if (typeof $1g === 'string') {
                $19 = $1g;
            };
            $1b = $1f;
            if ($1f && typeof $1f !== 'function' && (typeof $1f !== 'object' || typeof $1f.length !== 'number')) {
...
(function() {
    ...
    DayPilotCalendar.Calendar = function(id) {
        ...
        this.$2Z = function(i) {
            ...
            var $1g = {};
            for (var client_name in $o) {
                $1g[client_name] = $o[client_name];
            };
    ...
    DayPilotCalendar.Column = function($1I, client_name, $S) {
        this.value = $1I;
        ...
        (function($) {
            $.fn.daypilotCalendar = function($1J) {
                ...
                var j = this.each(function() {
                    ...
                    this.daypilot = $1L;
                    for (client_name in $1J) {
                        $1L[client_name] = $1J[client_name];
                    };
                    $1L.init();
                    if (!$1K) {
                        $1K = $1L;
        ...
        $1M.directive("daypilotCalendar", ['$parse', function($1N) {
            return {
                ...
                "link": function($1O, element, $1P) {
                    ...
                    for (var client_name in $1P) {
                        if (client_name.indexOf("on") === 0) {
                            (function(client_name) {
                                $b[client_name] = function($D) {
                                    var f = $1N($1P[client_name]);
                                    $1O["$apply"](function() {
                                        f($1O, {
                                            "args": $D
                                        });
                    ...
                    $1S.call($1O, $1T, function($1I) {
                        for (var client_name in $1I) {
                            $b[client_name] = $1I[client_name];
                        };
                        $b.update();
                        $b.$38();
                    }, true);
...
(function() {
    ...
    DayPilot.DatePicker = function($a) {
        ...
        this.prepare = function() {
            ...
            this.patterns = [];
            if ($a) {
                for (var client_name in $a) {
                    this[client_name] = $a[client_name];
                }
            };
            this.init();
        };
...
(function() {
    ...
    DayPilot.Modal = function($k) {
        ...
        this.$0z = function() {
            ...
            for (var client_name in $k) {
                this[client_name] = $k[client_name];
            }
        };
        this.$0z();
    };
    ...
    DayPilot.Modal.closeSerialized = function() {
        ...
        for (var i = 0; i < $06.length; i++) {
            var $07 = $06[i];
            var client_name = $07.client_name;
            if (!client_name) {
                continue;
            };
            var $08 = $07.value;
...
(function() {
    ...
    DayPilotMonth.Month = function($b) {
        ...
        this.$1n = function(i) {
            ...
            var $y = {};
            for (var client_name in $f) {
                $y[client_name] = $f[client_name];
            };
            if (typeof this.onBeforeEventRender === 'function') {
                var $o = {};
                $o.data = $y;
        ...
        (function($) {
            $.fn.daypilotMonth = function($0Z) {
                ...
                var j = this.each(function() {
                    ...
                    this.daypilot = $10;
                    for (client_name in $0Z) {
                        $10[client_name] = $0Z[client_name];
                    };
                    $10.Init();
                    if (!$09) {
                        $09 = $10;
        ...
        $11.directive("daypilotMonth", ['$parse', function($12) {
            return {
                ...
                "link": function($13, element, $14) {
                    ...
                    for (var client_name in $14) {
                        if (client_name.indexOf("on") === 0) {
                            (function(client_name) {
                                $c[client_name] = function($o) {
                                    var f = $12($14[client_name]);
                                    $13["$apply"](function() {
                                        f($13, {
                                            "args": $o
                                        });
                    ...
                    $17.call($13, $18, function($19) {
                        for (var client_name in $19) {
                            $c[client_name] = $19[client_name];
                        };
                        $c.update();
                    }, true);
...
(function() {
    ...
    DayPilot.Navigator = function(id) {
        ...
        this.$07 = function($e, client_name) {
            var $f = this.$02 ? this.$04("_" + client_name) : this.$04(client_name);
            DayPilot.Util.addClass($e, $f);
        };
        this.$08 = function($e, client_name) {
            var $f = this.$02 ? this.$04("_" + client_name) : this.$04(client_name);
        ...
        (function($) {
            $.fn.daypilotNavigator = function($U) {
                ...
                var j = this.each(function() {
                    ...
                    this.daypilot = $V;
                    for (var client_name in $U) {
                        $V[client_name] = $U[client_name];
                    };
                    $V.Init();
                    if (!$m) {
                        $m = $V;
                ...
                "compile": function compile(element, $X) {
                    ...
                    return function link($Y, element, $X) {
                        ...
                        $00.call($Y, $X["daypilotConfig"], function($01) {
                            for (var client_name in $01) {
                                $a[client_name] = $01[client_name];
                            };
                            $a.update();
                        }, true);
                    };
